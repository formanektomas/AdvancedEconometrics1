...
\begin{flalign*}
G(z_i & \rightarrow -\infty) & \rightarrow 0  \\
G(z_i & ~~= ~~0) & ~~= 0.5  \\
G(z_i & \rightarrow + \infty) & \rightarrow 1 
\end{flalign*}
\end{minipage}
\end{frame}
%---------------------------------
...
$\circled{$P_i $} = P(y_i = 1|\bm{x}_i) = \hat{y}_i = G(\bm{x}_i \bm{\beta}) = G(z)= \circled{$\frac{e^z}{1+e^z}$}$, \\ \medskip also:\\ \medskip
\medskip
...
\bigskip
where $\Gamma(\cdot)$ is the gamma function, defined for $r>0$ 
\vspace{-0.3cm}
\\by the following expression: 
$$\Gamma(r) = \int_0^{\infty} z^{r-1} \exp (-z) dz,$$
\medskip
and where $\mu_i = \exp (\bm{x}^{\prime} \bm{\beta}) = E(y_i|\bm{x}_i)$
\end{frame}
%---------------------------------
...
\begin{align*}
f(w_i|\bm{x}_i,c_i; \bm{\beta}, \sigma) =
\begin{cases}
(1/\sigma) \phi [(w_i - \bm{x}_i \bm{\beta})/\sigma] & \ \text{if } \mytikzmark{1}{w_i} < c_i \\
...
\draw[myarrow] (boxA) --++ (XA);
	...
	\draw[myarrow] (boxB) --++ (XB);
		\node[mynode] at (8.4,1.8)(box){\textcolor{Blue}{\underline{In the censored regression 				model, the coefficients}} \textcolor{Blue}{\underline{can be directly interpreted.}} This is 		contrary to the Tobit model. The censored 				regression model and the Tobit 	model have similar structures, but in the Tobit model, the 		outcome is of a nonlinear nature whereas in the censored regression model, the outcome is linear with respect to $\bm{\beta}$, but incompletely observed.};
\end{tikzpicture}
\end{frame}
%---------------------------------
...
\item In a truncated regression model, the outcome and regressors are only observed if the outcome is less or equal some value $c_i$
\item In this case, the sample is not a random sample from the population (systematically, some units will never be a part of the sample)
...
\end{frame}
%---------------------------------
\subsection{Sample selection corrections}
\begin{frame}{Sample selection corrections}
...
\end{frame}
%---------------------------------
\begin{frame}{Sample selection corrections (non-random selection)}
...
\end{frame}
%---------------------------------
\begin{frame}{Sample selection corrections: Heckit model}
