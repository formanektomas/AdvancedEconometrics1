...
\begin{frame}{Long panels -- models and estimation}
    \begin{itemize}
        ...
        \bigskip
        \item Long panels and the SURE / SUR estimator
        \bigskip
        \item SURE \& equations with identical regressors
        \bigskip
        \item SURE -- additional topics
    \end{itemize}
\end{frame}
%---------------------------------
...
\end{itemize}
    ...
    $\textnormal{var}(\bm{\varepsilon}|\bm{X}) = \bm{\Omega}$, where $\bm{\Omega}$ is a ($N\! \times \! N$) PSD matrix.
\end{itemize}
\end{frame}
%---------------------------------
\begin{frame}{Long panels -- models and estimation (BSc repetition)}
\footnotesize
General LRM: $\bm{y} = \bm{X\beta}+\bm{\varepsilon}$ ~~\&~~OLS vs FGLS:\\ \bigskip \bigskip
\begin{itemize}
    ...
    \item $\textnormal{var}(\bm{\varepsilon}|\bm{X})=\sigma^2\bm{H} \quad \rightarrow \quad$ use FGLS (efficent w.r.t. OLS):\\ \smallskip $$\bm{\hat{\beta}}=(\bm{X}^{\prime} \bm{H}^{-1} \bm{X})^{-1} \bm{X}^{\prime} \bm{H}^{-1} \bm{y}$$
    for empirical applications, we usually have to find some estimate of $\bm{H}$.
\end{itemize}
\end{frame}
%---------------------------------
\begin{frame}{Long panels -- models and estimation (BSc repetition)}
...
\begin{itemize}
    ...
    \dots given conforming dimensions of $\bm{A},\bm{C}$ and $\bm{B},\bm{D}$ matrices.
\end{itemize}
\end{frame}
%---------------------------------
\begin{frame}{Long panels -- models and estimation (BSc repetition)}
...
\begin{bmatrix}
\bm{A} & \bm{0} \\ \bm{0} & \bm{A}
\end{bmatrix}$\\ \smallskip i.e. the result is a block-diagonal matrix
...
\small
\begin{itemize}
    \item $\bm{y}_i = \bm{X}_i \bm{\beta}_i + \bm{\varepsilon}_i,\qquad i = 1,\dots M.$\\ \smallskip 
    ...
    FGLS is efficent w.r.t. OLS: uses information in $\bm{\Sigma} = [\sigma_{ij}]$.
\end{itemize}
\end{frame}
%---------------------------------
\begin{frame}{Long panels -- SUR models}
...
$\bm{\hat{\beta}}_{\textnormal{GLS}} = [\bm{X}^{\prime} \bm{\Omega}^{-1} \bm{X}]^{-1} \bm{X}^{\prime} \bm{\Omega}^{-1} \bm{y}$\\ \bigskip
    \begin{itemize}
    \item Homogeneity restrictions -- equal coefficients in all equations of the SUR model: $\bm{\beta}_1=\bm{\beta}_2=\cdots=\bm{\beta}_M$, \\ \smallskip i.e. $(M-1)K$ restrictions on the $(KM\times 1)$ vector $\bm{\beta}$ \\ \dots can be tested using Wald test (LR, LM tests).
    \bigskip
    ...
    \begin{itemize}
        ...
        \item SUR model with identical regressors ($\bm{X}_1=\bm{X}_2=\cdots=\bm{X}_M$): OLS and GLS are identical (discussed next).
    \end{itemize}
\end{itemize}
\end{frame}
%---------------------------------
...
Topic is mostly out of scope in terms of long-panel data. However, SUR models with identical regressors have important empirical applications: \\ \bigskip
\begin{itemize}
