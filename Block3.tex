...
\node[mynode] at (9.4,5.1) (box){RE approach is used because many of the variables are time-invariant. \underline{But is the random effects} \underline{assumption realistic?}};
...
\begin{itemize}
    ...
    \medskip
    \item GLS is efficent w.r.t. OLS: uses information on contemporaneous correlation among errors as in the matrix $\bm{\Sigma}= [\sigma_{ij}]$.
\end{itemize}
\end{frame}
%---------------------------------
\begin{frame}{Long panels -- general SUR models}
...
$\bm{\hat{\beta}}_{\textnormal{GLS}} = [\bm{X}^{\prime} \bm{\Omega}^{-1} \bm{X}]^{-1} \bm{X}^{\prime} \bm{\Omega}^{-1} \bm{y}$\\ \bigskip
    ...
    \end{itemize}
    \bigskip
    \item Homogeneity restrictions -- equal coefficients in all equations of the SUR model (analogous to `pooling OLS` model: $\bm{\beta}_1=\bm{\beta}_2=\cdots=\bm{\beta}_M$, \\ \smallskip i.e. $(M-1)K$ restrictions on the $(KM\times 1)$ vector $\bm{\beta}$ \\ \dots can be tested using Wald, LR and/or LM tests.
\end{itemize}
\end{frame}
%---------------------------------
...
\begin{itemize}
    ...
    $$
    where $r_{it}$ is the return of instrument $i$ over time period $t$, $r_{ft}$ and $r_{mt}$ describe risk-free and market returns respectively; $\alpha_i$ and $\beta_i$ are parameters, estimated separately for each $i$th financial isntrument -- same regressor $(r_{mt} - r_{ft})$ used in each regression equation.
\end{itemize}
\end{frame}
%---------------------------------
...
\begin{itemize}
    ...
    \bigskip
    \item General note: GLS computation assumes $\bm{\Sigma}$ is known, which is unlikely (with FGLS, $\bm{\Sigma}$ is estimated).
\end{itemize}
\end{frame}
%---------------------------------
...
\begin{itemize}
    ...
    \end{bmatrix} = \bm{X\beta}+\bm{\varepsilon}$\\
    \bigskip
    \item $\bm{\hat{\beta}}_{\textnormal{GLS}} = [\bm{X}^{\prime} (\bm{\Sigma} \otimes \bm{I}_T)^{-1} \bm{X}]^{-1} \bm{X}^{\prime} (\bm{\Sigma} \otimes \bm{I}_T)^{-1} \bm{y}$ \\ GLS estimator has the same form as in previous (``pooled'') case, yet $\bm{X}$ and $\bm{\beta}$ dimensions are different.
    \bigskip
    \item GLS computation assumes $\bm{\Sigma}$ is known, which is unlikely (with FGLS, $\bm{\Sigma}$ is estimated).
\end{itemize}
\end{frame}
%---------------------------------
...
\begin{itemize}
    ...
    $\hat{\sigma}_{ij} = \frac{1}{T}\bm{e}_i^{\prime}\bm{e}_j$ \quad and \quad $\hat{\bm{\Sigma}}=[\hat{\sigma}_{ij}]$ is estimated as follows: \medskip
    \begin{itemize}
    \item[1] SURE, model with identical $\bm{X}_i$ blocks: FGLS not relevant as GLS = equation-by-equation OLS. \\ \medskip
        \item[2] ``pooled'' case with identical $\bm{\beta}_i$ coefficients, where $\bm{X}$ is $(MT\times K)$:
        $\bm{e}_i$ is a subvector of OLS residuals from 
        $\bm{\hat{\beta}}_{\textnormal{OLS}} = [\bm{X}^{\prime}\bm{X}]^{-1} \bm{X}^{\prime} \bm{y}$.\\ \medskip
        \item[3] ``general case'' SURE:  $\bm{e}_i$ vectors come from equation-by-equation OLS: 
        $\bm{\hat{\beta}}_{i,\textnormal{OLS}} = [\bm{X}_i^{\prime}\bm{X}_i]^{-1} \bm{X}_i^{\prime} \bm{y}_i$,\\ or as subvector of $\bm{e}$ from OLS estimation of the stacked model: $\bm{\hat{\beta}}_{\textnormal{OLS}} = [\bm{X}^{\prime}\bm{X}]^{-1} \bm{X}^{\prime} \bm{y}$, where $\bm{X}$ is ($MT \times MK$) -- same residuals from both approaches. \medskip
        \medskip
    \end{itemize}
...
Swamy estimator:\\ \medskip
    \begin{itemize}
        ...
        and the $\hat{\bm{\Sigma}}_i$ elements (from $\hat{\sigma}^2_i$ and $\hat{\bm{\Omega}}_{\eta}$) can be obtained through separate OLS estimation across individual $i$-units.\\ \medskip
...
\begin{frame}{Cross-sectional dependence in large panels}
    \begin{itemize}
        ...
        \medskip
        \item \textbf{Spatial dependency approach:} discussed separately in the course 4EK417.\\ \textsubscript{ \textcolor{Blue}{\url{https://github.com/formanektomas/4EK417/raw/master/Block3/Block_3.pdf}} }
        \bigskip
...
\footnotesize
    \begin{itemize}
        ...
        $y_{it}=\bm{\alpha}_i^{\prime} \bm{d}_t + \bm{\alpha}_i^{\prime} \bm{x}_{it} + \bm{\gamma}_i^{\prime} \hat{\bm{f}}_{t} + e_{it}$, \\ \medskip
        ...
        \textsubscript{ \textcolor{Blue}{\url{https://github.com/formanektomas/4EK417/raw/master/Block2/Block_2.pdf}} }
        \bigskip \footnotesize
        \item In principle, this method aims at controlling for XSD through PCs, which leads to unbiased $\bm{\alpha}_i$ and $\bm{\alpha}_i$ estimates. However, if the PCs and regressors are correlated (a common case), this estimator becomes inconsistent. To solve this problem (and related issues), various modifications of this estimator exist.
...
\medskip
    \begin{itemize}
        ...
        \item assumption of cross-sectional is often violated,
