...
\begin{itemize}
    ...
    $$
    where $r_{it}$ is the return of instrument $i$ over time period $t$, $r_{ft}$ and $r_{mt}$ describe risk-free and market returns respectively; $\alpha_i$ and $\beta_i$ are parameters, estimated of each $i$th financial isntrument -- same regressor $(r_{mt} - r_{ft})$ used in each regression equation.
\end{itemize}
\end{frame}
%---------------------------------
...
\begin{itemize}
    ...
    \begin{itemize}
        \item[1] SURE, $\bm{X}$ is block diagonal with general $\bm{X}_i$ blocks:\\
        $\hat{\bm{\Sigma}}$ for the general SUR model:  $\bm{e}_i$ vectors come from equation-by-equation OLS: 
        ...
        \item[2] SURE, $\bm{X}$ is block diagonal with identical $\bm{X}_i$ blocks:\\ 
        SUR model with identical regressors: FGLS not relevant as GLS = equation-by-equation OLS (shown previously). \\ \medskip
        \item[3] SURE, identical $\bm{\beta}_i$, i.e. $\bm{X}$ with $\bm{X}_i$ stacked vertically:  \\ $\hat{\bm{\Sigma}}$ for the ``pooled'' SUR model: 
