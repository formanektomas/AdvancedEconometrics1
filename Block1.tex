...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \smallskip
        \item \textbf{ML:} For any two parameter vectors $\bm{\theta} \neq \bm{\theta}_0$, a data vector $(y_i, \bm{x}_i)$ must exist, which generate different values of density function: $f(y_i|\bm{x}_i,\bm{\theta}) \neq f(y_i|\bm{x}_i,\bm{\theta}_0)$. \\ \smallskip Note: identifiability does not rule out \\$f(y_i|\bm{x}_i,\bm{\theta}) = f(y_{\ell}|\bm{x}_{\ell},\bm{\theta}), \textnormal{~~where},~~y_i=y_{\ell},~~\bm{x}_i \neq \bm{x}_{\ell}$ \\(but the A2 full rank condition must be observed).
        \smallskip
