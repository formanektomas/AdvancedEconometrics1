...
If: \\ \smallskip (a) the parameter space is convex and the true parameter vector is a point in its interior,\\(b) the criterion function is concave,\\(c) the parameters are identified by the criterion function, and\\(d) the data are well behaved,\\ \smallskip then the M estimator converges in probability to the true parameter vector. 
\end{block}
\end{frame}
%---------------------------------------------
\begin{frame}{Estimators and estimation methods}
...
If: \\ \smallskip 
(a) $\hat{\bm{\theta}}$ is a consistent estimator of $\bm{\theta}_0$ where $\bm{\theta}_0$ is a point in the interior of the parameter space $\bm{\Theta}$, \\  \smallskip 
(b) $q(\bm{\theta}|\textbf{data})$ is concave and twice continuously differentiable in $\bm{\theta}$ in a neighborhood of $\bm{\theta}_0$, \\  \smallskip 
(c) $\sqrt{n} \left[ \partial q(\bm{\theta}_0|\textbf{data})/ \partial \bm{\theta}_0 \right] \xrightarrow{~~d~~} N(\bm{0},\bm{\Phi}),$ \\  \smallskip 
(d) $\underset{n \rightarrow \infty}{\lim} \textnormal{Pr} \left[ |(\partial^2 q(\bm{\theta}|\textbf{data})/ \partial \theta_k \partial \theta_m)- h_{km}(\bm{\theta}) | >\varepsilon \right] = 0~ \forall \varepsilon > 0$ for any $\bm{\theta}$ in $\bm{\Theta}$, where $h_{km}(\bm{\theta})$ is a continuous finite valued function of $\bm{\theta}$, \\  \smallskip 
(e) the matrix of elements $\bm{H}(\bm{\theta})$ is nonsingular at $\bm{\theta}_0$,\\
