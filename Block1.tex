...
%---------------------------------------------
\begin{frame}{Method of moments}
\textbf{LRM estimation by MM:} \\ \medskip
%\footnotesize
\begin{itemize}
...
\begin{itemize}
    ...
    \item We can use elements on the main diagonal of  $\textnormal{var}(\hat{\bm{\theta}})$ to test null hypotheses concerning individual parameters (analogous to statistical inference used in OLS).
\end{itemize}
\end{frame}
%---------------------------------------------
\begin{frame}{Maximum likelihood estimator}
...
2 \cdot \sum_{i=1}^n \, [y_i-h(\bm{x}_i, \bm{\beta})] \,
...
\begin{itemize}
\item CLRM and its estimate (for $i=1,\dots,n$):
\begin{align}\nonumber
y_i & = \beta_1 + \beta_2 x_{i2} +\beta_{3} x_{i3} + \dots + \beta_K x_{iK} + \varepsilon_i \\ \nonumber
\hat{y}_i & = \hat{\beta}_1 + \hat{\beta}_2 x_{i2} +\hat{\beta}_3 x_{i3} + \dots + \hat{\beta}_K x_{iK} \\ \nonumber
\end{align}
\item Prediction of expected value ($\bm{x}_p^{\prime}$ can be out of sample): 
\begin{align}\nonumber
\hat{y}_p & = E(y|x_{p1} = 1, x_{p2} = c_2,\dots,x_{pK} = c_K)\\ \nonumber
...
$$y^*=\beta^*_1 + \beta^*_2 (x_2 -c_2) + \beta^*_3(x_3 - c_3) + \dots + \varepsilon$$
...
\begin{align}\nonumber
\hat{y}_p &  = \hat{\beta}_1 + \hat{\beta}_2 c_2 + \hat{\beta}_3 c_3 + \dots + \hat{\beta}_K c_K\\ \nonumber
y_p & = \beta_1 + \beta_2 c_2 + \beta_3 c_3 + \dots+\beta_K c_K + \varepsilon_p \nonumber
\end{align} 
\item Prediction error
$$\hat{e}_p = y_p - \hat{y}_p = (\beta_1 + \beta_2 c_2 + \beta_3 c_3 + \dots+\beta_K c_K) + \varepsilon_p - \hat{y}_p$$
\item Prediction error variance
$$\textnormal{var}(\hat{e}_p) = \textnormal{var}(\varepsilon_p) + \textnormal{var}(\hat{y}_p)$$
...
\begin{frame}{Predictions - basics}
\begin{itemize}
\item In CLRM, as homoscedasticity holds, $\sigma^2 = \textnormal{var}(\varepsilon_p)$: \\
...
\begin{align}\nonumber
\log(y) &= \beta_1 + \beta_2 x_2 + \dots + \beta_K x_K + \varepsilon \\ \nonumber
\widehat{\log(y)} &= \hat{\beta}_1 + \hat{\beta}_2 x_2 + \dots + \hat{\beta}_K x_K 
\end{align}
$\hat{y} = \exp (\widehat{\log(y)}) = e^{\widehat{\log(y)}}$ systematically underestimates $\hat{y}$ , \\
\vspace{0.3cm}
we can use a correction: $\hat{y}=\widehat{\alpha}_0 e^{\widehat{\log(y)}}$ \\
\vspace{0.3cm}
where $\widehat{\alpha}_0 = n^{-1} \sum_{i=1}^n \exp(\hat{\varepsilon}_i)$ \\
\vspace{0.3cm}
is a consistent (not unbiased) estimator of $\exp{(\varepsilon)}$.
\end{itemize}
\end{frame}
...
\vspace{0.3cm}
Difference between actual $y_p$ and prediction value:\\
\vspace{0.3cm}
$\hat{e}_p \,=\,  y_p - \hat{y}_p 
  \,=\,(\bm{x}_{p}^\prime \bm{\beta} + \varepsilon_p) - \bm{x}_{p}^\prime \hat{\bm{\beta}} 
  \,=\, \bm{x}_{p}^\prime (\bm{\beta} - \hat{\bm{\beta}}) + \varepsilon_p$\\
\vspace{0.3cm}
If $\hat{\bm{\beta}}$ is unbiased estimator for $\bm{\beta}$, \\
$\hat{y}_p$ is an unbiased estimator for $y_p$ value:\\
\vspace{0.3cm}
$E(\hat{e}_p) \,=\, E(y_p - \hat{y}_p)
   \,=\, \bm{x}_{p}^\prime E(\bm{\beta} - \hat{\bm{\beta}}) + E(\varepsilon_p) =0$\\
\vspace{0.3cm}
and the variance of $\hat{e}_p$ can be expressed as:\\
\vspace{0.3cm}
$E(\hat{e}_p^2) \,=\,\textnormal{var}(\hat{e}_p)
   \,=\, \bm{x}_{p}^\prime \textnormal{var}(\hat{\bm{\beta}})\bm{x}_{p} + \textnormal{var}(\varepsilon_p) $
\end{frame}
...
\textnormal{var}(\hat{e}_p) \,&=\,
  \bm{x}_{p}^\prime \textnormal{var}(\hat{\bm{\beta}})\bm{x}_{p} + \textnormal{var}(\varepsilon_p)\\
 \,&=\,
   \bm{x}_{p}^\prime \left[ \sigma^2 \left( \bm{X}^\prime \! \bm{X}  \right)^{-1} \right] \bm{x}_{p} + \textnormal{var}(\varepsilon_p)\\
   & \textnormal{substitute $\sigma^2 = \textnormal{var}(\varepsilon_p)$, with $\hat{\sigma}^2$ (homoscedasticity)}\\
  \,&=\,
...
$\textnormal{var}(\hat{e}_p) = \hat{\sigma}_p^2 + \hat{\sigma}^2$ converges to $\hat{\sigma}^2$\\
$\dots$ because  $\textnormal{plim} \, \hat{\bm{\beta}} = \bm{\beta} \,\,\,\, \leftrightarrow \,\,\,\, 
...
Variance of $\hat{e}_p$ (continued):
$$
\qquad \textnormal{var}(\hat{e}_p) \,=\,
\bm{x}_{p}^\prime \left[ \hat{\sigma}^2 \left( \bm{X}^\prime \! \bm{X}  \right)^{-1} \right] \bm{x}_{p} + \hat{\sigma}^2.
$$
\begin{itemize}
    \item \textbf{Individual prediction error:} after re-arranging, the above expression can be cast for $\textnormal{s.e.}(\hat{e}_p)$ as
    $$
    ~\textnormal{s.e.}(\hat{e}_p) \,=\, \hat{\sigma} \cdot \,  
   \sqrt[]{1+ \bm{x}_{p}^\prime \left( \bm{X}^\prime \! \bm{X}  \right)^{-1} \bm{x}_{p}~} \,,
    $$
    \item \textbf{Mean prediction error:} calculated considering $\hat{\sigma}_p^2$ only
    $$
...
%---------------------------------------------
\begin{frame}{Predictions - basics}
\textbf{Prediction intervals:} \\ \smallskip Individual vs. mean value prediction intervals:\\
\vspace{0.3cm}
...
then the $\textbf{expected test-sample square error}$ (loss function) can be decomposed into:\\
\bigskip
$$\textit{MSE}_0
...
$\textnormal{Bias\,}(\hat{f}(\bm{x}_0)) 
       = E[\hat{f}(\bm{x}_0)]
       - f(\bm{x}_0)$,\\
$\varepsilon_0$ is the irreducible error: $\textit{MSE}_0 \geq \varepsilon_0$,\\ 
