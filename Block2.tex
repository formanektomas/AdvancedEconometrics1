...
$$
\hat{\bm{\beta}}~~ \underset{\textnormal{as.}}{\sim}~~ N(\bm{\beta}, \hat{\sigma}^2(\bm{X}^{\prime} \bm{X})^{-1} )
\quad \textnormal{~as~} n \rightarrow \infty
$$
\end{itemize}
\end{frame}
%------------------------------------------------------
\begin{frame}{Serial correlation of the error term in TSRM}
\begin{itemize}
...
%------------------------------------------------------
\begin{frame}{Serial correlation of the error term in TSRM}
Breusch-Godfrey (BG) test for serial correlation with general regressors (contemporaneous exogneity holding). Large sample validity only. Regressors from main equation are used in the auxiliary regression for the BG test:\\ \medskip
\begin{itemize}
\item BG test for an ar(1) process in residuals:
...
\item BG test for ar($q$) serial correlation in residuals:
...
Use robust $F$-test \quad or \quad LM-test: $(n-q)R_a^2 \underset{H_0}{\sim} \chi^2_{(q)}$.
\end{itemize}
\end{frame}
%------------------------------------------------------
...
\begin{itemize}
    \item[a)] Start by repeated OLS estimation of the generalized first differences specification, using different (fixed) values of $\rho \in (-1,1)$, to get alternative $\bm{\hat{\beta}}_{\rho}$ vectors at each $\rho$. 
    \item[b)] From such auxiliary regressions (i.e. for different $\rho$ values), select the $\bm{\hat{\beta}}_{\rho}$ which minimizes RSS.
    \medskip
...
\item In model $(6)$, we lose information linked with the levels of variables, short term relations are stressed.
...
\end{equation}
%~\\         
\textbf{Note:} ADLs are versatile. For example, restrictions can be imposed on the ADL$(1,1)$: $\beta_1 = 1$ and $\gamma_1 = -\gamma_0$, and we get a model in $1^{st}$ diffs.:
$$\Delta y_t = \beta_0 + \gamma_0 \Delta x_t + u_t\,. $$
\end{itemize}
\end{frame}
%---------------------------------------------
\begin{frame}{Error correction model (ECM)}
...
For simple static regression between two cointegrated variables: $y_t, x_t \sim CI(1,1)$, super-consistency applies (with deterministic regressors such as intercept and trend added upon relevance). Dynamic misspecifications do not necessarily have serious consequences. This is a large sample property - in small samples, OLS estimators are biased.\\ (Specific statistical inference applies to cointegrating vectors.)
\end{block}
\end{frame}
%---------------------------------------------
\begin{frame}{ECM: non-stationary \& cointegrated series}
